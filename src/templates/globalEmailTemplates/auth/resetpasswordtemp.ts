...
export const OtpTemplate = (fullName: string, token: string) => {
  const head = emailHeader();
  const foot = emailFooter();
  ...
  const body = `
   ...
   <div style="font-size: x-large; letter-spacing: 4px; color: #fff; font-weight: 400; text-align: center;"> ${token}</div>
</main>
    `;
