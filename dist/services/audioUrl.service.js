...
exports.normalizeQuranAudioUrl = exports.buildQuranAudioUrl = exports.getGlobalAyahNumber = void 0;
...
const normalizeBitrate = (bitrate) => {
    const raw = bitrate === undefined || bitrate === null
...
const normalizeReciterId = (reciterId) => {
    if (!reciterId || typeof reciterId !== "string") {
        throw new Error("Invalid reciterId for Quran audio URL");
    }
    const value = reciterId.trim();
    if (!/^ar\./.test(value)) {
        throw new Error(`Invalid Quran reciter id: ${reciterId}. Expected a reciter id like ar.alafasy`);
    }
    return value;
};
const getGlobalAyahNumber = (surahNumber, ayahNumber) => {
    const parsedSurah = Number(surahNumber);
    const parsedAyah = Number(ayahNumber);
    if (!Number.isInteger(parsedSurah) ||
        parsedSurah <= 0 ||
        parsedSurah > SURAH_AYAH_COUNTS.length) {
        throw new Error(`Invalid surah number: ${surahNumber}. Expected a value between 1 and ${SURAH_AYAH_COUNTS.length}.`);
    }
    if (!Number.isInteger(parsedAyah) || parsedAyah <= 0) {
        throw new Error(`Invalid ayah number: ${ayahNumber}. Expected a positive integer.`);
    }
    let offset = 0;
    ...
    if (!surahCount || parsedAyah > surahCount) {
        throw new Error(`Invalid ayah number ${ayahNumber} for surah ${surahNumber}.`);
    }
    return offset + parsedAyah;
};
exports.getGlobalAyahNumber = getGlobalAyahNumber;
const buildQuranAudioUrl = (reciterId, surahNumber, ayahNumber, bitrate) => {
    const parsedReciterId = normalizeReciterId(reciterId);
    const globalAyahNumber = (0, exports.getGlobalAyahNumber)(surahNumber, ayahNumber);
...
exports.buildQuranAudioUrl = buildQuranAudioUrl;
const normalizeQuranAudioUrl = (audioUrl, reciterId, surahNumber, ayahNumber, bitrate) => {
    try {
        ...
        const bitratePart = slicedParts[0];
        const reciterPart = slicedParts[1];
        const third = slicedParts[2];
        const fourth = slicedParts[3];
        const configuredBitrate = normalizeBitrate(bitrate);
        ...
        if (bitratePart.startsWith("ar.") &&
            ...
            if (Number.isInteger(sourceSurah) && Number.isInteger(sourceAyah)) {
                const globalAyahNumber = (0, exports.getGlobalAyahNumber)(sourceSurah, sourceAyah);
...
exports.normalizeQuranAudioUrl = normalizeQuranAudioUrl;
//# sourceMappingURL=audioUrl.service.js.map